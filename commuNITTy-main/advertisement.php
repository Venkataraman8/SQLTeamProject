<?php
	...
	if(isset($_SESSION['username']))
	{
                ...
                if (isset($_POST['post'])) {
                    $username = $_SESSION['username'];
                    $ad_title = mysqli_real_escape_string($connect, $_POST['ad_title']);
                    $ad_desc = mysqli_real_escape_string($connect, $_POST['ad_desc']);
                    ...
                    $count = $count + 1;
                    $p_id = $username . "P_" . $count;

                    $query1 = "insert into $username (id) values ( '$p_id')";
                    $res1 = mysqli_query($connect, $query1);

                    $query0= "create table if not exists product(  p_id varchar (250), username varchar(250), ad_title varchar(1000), ad_desc varchar(1000), keyword varchar(250))";
                    $res0= mysqli_query($connect, $query0);
                    
                    $query01 = "insert into product( p_id, username, ad_title, ad_desc, keyword) values ('$p_id', '$username', '$ad_title','$ad_desc','$keyword')";
...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
    <!-- Navbar content -->
    <a class="navbar-brand" href="#">CommuNITTy</a>
...
<div align="center" id="h2"><h2><u>Posting an Advertisement!</u></h2></div>
     <div id="body1">
            <br>
              <?php
                  if ($gen!=1 && $set!=1 ){
                      echo '<form method="POST">';
               echo' <div align="center"> Advertisement About? <br><input type="text" name="ad_title" style="font-size: 17px; height: 60px; width:600px;" required></div>
                    <div align="center"> Advertisement Description. <br><input type="text" name="ad_desc" style="font-size: 17px; height: 60px; width:600px;" required></div>
                            <div align="center" >
                            <br>
